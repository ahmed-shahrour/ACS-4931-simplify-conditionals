...
def good_standing(total_cholostrol, ldl, triglyceride):
    return total_cholostrol < 200 \
        and ldl < 100 \
        and triglyceride < 150


def requires_cautious(total_cholostrol, ldl, triglyceride):
    return 200 < total_cholostrol > 240 \
        or ldl > 160 \
        or triglyceride >= 200


def requires_tlc_diet(total_cholostrol, ldl, triglyceride):
    return 200 < total_cholostrol < 240 \
        or 130 < ldl < 160 \
...
if good_standing(total_cholostrol, ldl, triglyceride):
    print('*** Good level of cholestrol ***')
elif requires_cautious(total_cholostrol, triglyceride):
    print('*** High cholestrol level ***')
    print('start taking pills such as statins')
    print('start TLC diet')
elif requires_tlc_diet(total_cholostrol, triglyceride):
